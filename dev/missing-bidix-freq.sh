...
cat $INPUT | grep '<v' | grep '@' | cut -f2 -d'@' | cut -f1 -d'<' | lt-proc -w $DEV/../../mt-he.automorf.bin | cg-proc /tmp/temp_cg.bin  | cut -f2- -d'/' | sed 's/<pres><p3><sg>//g' | grep '<v' | cut -f1 -d'$' | sort -f | uniq -c | sort -gr  | grep -v '[0-9] $' > $DEV/pending_verbs.txt
...
cat $INPUT | grep '@' | grep '<adj>'  | sed 's/<\(f\|mf\|ma\|nt\|du\|m\|sg\|pl\|acc\|nom\|ins\|loc\|gen\|dat\|voc\)>//g' | sed 's/<\(ind\|def\)>//g' | sh ~/scripts/lowercase.sh  | sed 's/[ανρχί]/\n/g' | sed 's/^\W*\^/^/g' | sort -f | uniq -c  | sort -gr  | grep -v '[0-9] $' | grep '<adj>' | grep '@' > $DEV/pending_adjectives.txt

cat $INPUT | grep '@' | grep '<n>'  | sed 's/ה<det><def>+//g' | sed 's/\(<pl>\|<du>\|<sg>\|<ins>\|<acc>\|<dat>\|<loc>\|<gen>\|<dir>\|<nom>\|<px1sg>\|px2sg\|px3sg\|px3sg_m\|px3sg_f\|px1pl\|px2pl\|px3pl\|<abl>\|<abs>\|<1s>\|<2s>\|<3s>\|<1p>\|<2p>\|<3p>\|<p1s>\|<p2s>\|<p3s>\|<p1p>\|<p2p>\|<p3p>\)//g' | sed 's/<pl>//g' | sh ~/scripts/lowercase.sh  | sed 's/^\W*\^/^/g' | sed 's/<nd>/<ND>/g' | sed 's/<gd>/<GD>/g' | sort -f | uniq -c  | sort -gr  | grep -v '[0-9] $'  > $DEV/pending_nouns.txt

cat $INPUT | grep '@' | grep '<prn>'  | sed 's/\(<pl>\|<du>\|<sg>\|<ins>\|<acc>\|<dat>\|<loc>\|<gen>\|<dir>\|<nom>\|<abl>\|<abs>\|<1s>\|<2s>\|<3s>\|<1p>\|<2p>\|<3p>\|<p1s>\|<p2s>\|<p3s>\|<p1p>\|<p2p>\|<p3p>\)//g' | sed 's/<pl>//g' | sh ~/scripts/lowercase.sh  | sed 's/^\W*\^/^/g' | sort -f | uniq -c  | sort -gr  | grep -v '[0-9] $'  > $DEV/pending_pronouns.txt
...
cat $INPUT | grep '@' | grep '<det>'  | grep -v '<n>' | sed 's/<sg>//g' | sed 's/<pl>//g' | sed 's/^\W*\^/^/g' | sort -f | uniq -c  | sort -gr  | grep -v '[0-9] $' > $DEV/pending_determiners.txt
