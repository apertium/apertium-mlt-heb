...
NEEDED=`calc "$NUMWORDS*.80-$NUMKNOWNWORDS"`
echo "Tokens needed to get 80 %: $NEEDED . Corresponding wordlist in $N"
awk '// {print $0; t += $1; if( t > '"$NEEDED"' ) exit; } END {print t}' $S > $N
#awk '// {printf "%s", $0; w += 1; t += $1; if(w%1==0) printf "\n"; if( t > '"$NEEDED"' ) exit; } END {print t}' $S

# full word list, useful:
sort -f $F |uniq -c | sort -gr >/tmp/corpus-stat-all-sorted.txt 
